...
abstract class TimRestInterface
{
	 ...
	 * @param array $role 成员身份
	 * @return array 通过解析REST接口json返回包得到的关联数组，包含拉取到的群组成员详细信息(如果成功)、成功与否、错误提示等字段
	 */
	abstract function group_get_group_member_info($group_id, $limit, $offset,$role);
	...
	//查询账号
	abstract function AccountGroup($arrUserid);
	//查询账号在线情况
	abstract function AccountStateGroup($arrUserid);

	//批量导入用户
    abstract function multiaccount_import($arrUserList);

    //获取直播间在线人数
    abstract  function get_online_member_num($GroupId);
