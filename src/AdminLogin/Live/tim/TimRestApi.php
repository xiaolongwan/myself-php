...
class TimRestAPI extends TimRestInterface
{
	...
	public function generate_user_sig($identifier, $expiry_after, $protected_key_path, $tool_path)
	{
		...
		//echo $sign;
		var_dump($out);
		$this->usersig = isset($out[0])?$out[0]:'';
		return $out;
	}
...
//		}
    ...
    public function AccountGroup($arrUserid)
    {
        ...
        $ret = $this->api("im_open_login_svc", "account_check", $this->identifier, $this->usersig, $req_data);
        $ret = json_decode($ret, true);
        return $ret;
    }
    //查询账号状态
    ...
    //查询账号状态
    public function multiaccount_import($arrUserid)
    {
        ...
        $ret = $this->api("im_open_login_svc", "multiaccount_import", $this->identifier, $this->usersig, $req_data);
        $ret = json_decode($ret, true);
        return $ret;
    }
	...
	function group_get_group_member_info($group_id, $limit, $offset,$role =['Owner','Admin','Member'])
	{
    ...
    function shut_up($account_id,$time)
    {
     ...
     //获取直播群在线人数
    function get_online_member_num($groupId)
    {
        #构造新消息
        $msg = array(
            "GroupId" => $groupId,
        ...
        $ret = $this->api("group_open_http_svc", "get_online_member_num", $this->identifier, $this->usersig, $req_data);
        $ret = json_decode($ret, true);
        return $ret;
    }
